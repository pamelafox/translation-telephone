...
export class TranslationsFooter extends LitElement {
  ...
  render() {
    ...
    return html`<div>
         ...
         <p>▶ React:
           <button @click=${() => this.react('deeep')} ?disabled=${this._reactions.includes('deeep')}>🤔 Deeep!</button>
           <button @click=${() => this.react('funny')} ?disabled=${this._reactions.includes('funny')}>😆 Funny!</button>
           <button @click=${() => this.react('flags')} ?disabled=${this._reactions.includes('flags')}>🚫 Offensive</button>
         </p>
         <p>▶ Share:
           <input type="text" readonly="" size="70" value=${url}></p>
