...
def rounds():
  if request.method == 'POST':
    ...
    if order == '-views' and num < 5: # Get 5 random popular ones
      rounds = RoundModel.query.filter(RoundModel.usergen==True).order_by(RoundModel.views.desc()).limit(30)
      random.shuffle(rounds)
      rounds = rounds[0:num]
    elif order == '-views':
      rounds = RoundModel.query.filter(RoundModel.usergen==True).order_by(RoundModel.views.desc()).limit(num)
    else:
      rounds = RoundModel.query.filter(RoundModel.usergen==True).order_by(RoundModel.date.desc()).limit(num)
